...
#convert all columns
new_moral_issues <- type.convert(moral_issues)
...
###Moral issues pivot longer####

moral_issues %>% 
  select(-prop, -n) %>% 
  pivot_longer(cols = c("prop_lib", "prop_cons"),
               names_to = "polviews", 
               values_to = "prop_specific")

moral_issues %>% 
  select(-prop, -n) %>% 
  pivot_longer(cols = c("prop_lib", "prop_cons"),
               names_to = "polviews", values_to = "prop") %>% 
  filter(!is.na(prop)) #filter out NAs
