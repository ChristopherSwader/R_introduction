...
####Running the model####
my_model <- lm(patr_mean ~ log_gdp, data = flfp_agg)
my_model
...
#interaction terms####
mod_int <- lm(wvs_flfp ~ patr_mean + muslim + patr_mean:muslim, data = flfp_agg)
tidy(mod_int)
...
mod_val_cntr <- glm(lfp ~ patr_values +   patr_mean,
                    family = binomial(link = "logit"),
                    data = flfp)
tidy(mod_val_cntr, exponentiate = TRUE)
tidy(mod_val_cntr, exponentiate = F) #matches the model summary

#note that you would get a warning written within the model summary if you woudl add cntry to the above model
#because cntry and patr_mean are perfectly collinear. 

#checking for multicollinearity
cor(flfp[,c("patr_values", "patr_mean")])
